...
class QueryBuilder
{
   ...
   {
      $stmt = $this->query();  // Préparer la requête une seule fois
      $stmt->execute($this->parameters);  // Exécution
      return $stmt->fetch(PDO::FETCH_ASSOC);  // Retourner le résultat
   }
   ...
   public function fetchAll(): array
   {
      $stmt = $this->query();  // Préparer la requête une seule fois
      $stmt->execute($this->parameters);  // Exécution
      return $stmt->fetchAll(PDO::FETCH_ASSOC);  // Retourner tous les résultats
   }
